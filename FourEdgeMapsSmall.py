...
def getInLatencies(in_pattern, num_in_neurons, early_latency_list, late_latency_list,
    ...
    in_pattern_list = ["category0", "category1", "category2", "category3"]
    ...
    if in_pattern == "category0":
        for i in [0, 1, 6, 7, 8, 13]:
            latency_early = \
    ...
    elif in_pattern == "category1":
        for i in [2, 4, 5, 8, 14, 15]:
            latency_early = \
                BimodalLatency("early", mean_early, std_early, mean_late, std_late, low_lim, high_lim)
            InLatencies[i] = latency_early
            early_latency_list.append(latency_early)

    elif in_pattern == "category2":
        for i in [9, 10, 13, 14]:
            latency_early = \
                BimodalLatency("early", mean_early, std_early, mean_late, std_late, low_lim, high_lim)
            InLatencies[i] = latency_early
...
#%% Parameters to tune
######################################################################################
printout_dir = "sim_printouts/FourEdgeMapsSmall/"

## Specify Global Connectivity Parmeters
num_neurons_perLayer = [16, 8, 4]       # Assuming num_neurons_perLayer is the number of connections in FC case
num_connect_perNeuron = [1,8,-1]        # -1 denotes FC       

max_num_fires = 1

fan_in_neuron = [
                    *[[]] * int(num_neurons_perLayer[0]),
                    *[[0, 1, 4, 5, 8, 9, 12, 13]] * int(num_neurons_perLayer[1]/2),
                    *[[2, 3, 6, 7, 10, 11, 14, 15]] * int(num_neurons_perLayer[1]/2),
                    *[[x for x in range(16, 24)]] * num_neurons_perLayer[2]  
                ]

initial_weight_input = [10] * num_neurons_perLayer[0]
...
input_patterns = ("category0", "category1", "category2", "category3")

output_pattern = \
    {
        "category0"     :   sum(num_neurons_perLayer[0:-1]),
        "category1"     :   sum(num_neurons_perLayer[0:-1]) + 1,
        "category2"     :   sum(num_neurons_perLayer[0:-1]) + 2,
        "category3"     :   sum(num_neurons_perLayer[0:-1]) + 3
    }
