...
def initializeNetWorkConnectivity_sparse_output(
        num_categories, num_edge_maps, W_input, F_hidden, S_hidden,
        num_slices_per_class, slice_stride,
        weight_vector, sheet_dir
):
    ## This function now implements sparse connection between hidden and output layer
    ## each output neuron will only connect to "num_slices_per_class" slices of hidden neuron
    ## at each sublocation
...
#############################################################################################
    # a list of indicies for output layer neurons
    output_neuron_list = [idx for idx in
                          range(num_input_neurons+num_hidden_neurons,
                                num_input_neurons+num_hidden_neurons+num_categories)
                          ]

    # a list of indices for hidden layer neurons
    hidden_neuron_list_flattened = [idx for idx in
                                    range(num_input_neurons,
                                          num_input_neurons+num_hidden_neurons)
                                    ]
...
###################### Establish Output Layer Connectivity ##################
    ## a list of indicies for hidden layer neurons
    ## hidden_neuron_list[sublocation_idx][slice_idx]
    ## hidden_neurons are grouped in sublocation-major order
    hidden_neuron_list = [
        [None for slice_idx in range(num_hidden_slices)]
        for sublocation_idx in range(num_hidden_neurons_per_slice)
    ]
    for sublocation_idx in range(num_hidden_neurons_per_slice):
        for slice_idx in range(num_hidden_slices):
            hidden_neuron_list[sublocation_idx][slice_idx] = \
                num_input_neurons + sublocation_idx * num_hidden_slices + slice_idx


    ## for each output neuron, establish their fan-in slice_idx
    ## output_fan_in_slice_idx[neuron_idx][slice_idx]
    output_fan_in_slice_idx = \
        [
            [None for slice_idx in range(num_slices_per_class)]
            for neuron_idx in range(num_output_neurons)
        ]
    for neuron_idx in range(num_output_neurons):
        starting_slice_idx = neuron_idx * slice_stride
        output_fan_in_slice_idx[neuron_idx] = \
            list(range(starting_slice_idx,
                       starting_slice_idx + num_slices_per_class))
        ...
        [
            {
                ...
                "fan_out_synapse_indices":  [],
                "fan_out_neuron_indices":   []
            } for output_neuron in range(num_output_neurons)
        ]
    first_neuron_idx_output = num_input_neurons + num_output_neurons
    for idx in range(num_output_neurons):
        output_connectivity[idx]["neuron_idx"] = idx + first_neuron_idx_output
        ...
        for sublocation_idx in range(num_hidden_neurons_per_slice):
            for slice_idx in output_fan_in_slice_idx[idx]:
                output_connectivity[idx]["fan_in_neuron_indices"].append(
                    hidden_neuron_list[sublocation_idx][slice_idx]
                )
        ...
        output_connectivity[idx]["fan_in_synapse_addrs"] = \
            [
                ...
                range(first_fan_in_synapse_output + idx * num_slices_per_class*num_hidden_neurons_per_slice,
                    first_fan_in_synapse_output + (idx + 1) * num_slices_per_class*num_hidden_neurons_per_slice)
            ]
...
###################### Establish Hidden Layer Connectivity ##################
    ...
    for sublocation_idx in range(num_hidden_neurons_per_slice):
        for slice_idx in range(num_hidden_slices):
            for edge_map_idx in range(num_edge_maps):
                presynaptic_input_neuron_indices[sublocation_idx][slice_idx][
                edge_map_idx * F_hidden ** 2: (edge_map_idx + 1) * F_hidden ** 2] = \
                    list(map(lambda x: edge_map_idx * W_input ** 2 + x, receptive_pixel_indices[sublocation_idx]))
                fan_in_synapse_addrs[sublocation_idx][slice_idx][
                edge_map_idx * F_hidden ** 2: (edge_map_idx + 1) * F_hidden ** 2] = \
                    list(map(lambda x: ((
                                            sublocation_idx * num_hidden_slices + slice_idx) * num_edge_maps + edge_map_idx) * (
                                            F_hidden ** 2) + num_input_neurons + x,
                             [element for element in range(F_hidden ** 2)]))
    ...
    for idx in range(num_hidden_neurons):
        ...
        sublocation_idx, slice_idx = divmod(idx, num_hidden_slices)
        hidden_connectivity[idx]["slice_idx"] = slice_idx
        hidden_connectivity[idx]["sublocation_idx"] = sublocation_idx

        fan_in_synapses = fan_in_synapse_addrs[sublocation_idx][slice_idx]
        hidden_connectivity[idx]["fan_in_synapse_addrs"] = fan_in_synapses

        fan_in_neurons = presynaptic_input_neuron_indices[sublocation_idx][slice_idx]
        hidden_connectivity[idx]["fan_in_neuron_indices"] = fan_in_neurons

        fan_out_class_idx, slice_group_idx = \
            index_2d_multirows(output_fan_in_slice_idx, slice_idx)
        hidden_connectivity[idx]["fan_out_neuron_indices"] = \
            list(map(lambda x: x+num_input_neurons+num_hidden_neurons_per_slice,
                     [class_idx for class_idx in fan_out_class_idx]
                     )
                 )

        # with each fan-out neuron, there is a fan-out synapse to be connected
        for i in range(len(fan_out_class_idx)):
            fan_out_synapse_idx = \
                first_fan_in_synapse_output + \
                fan_out_class_idx[i] * num_slices_per_class * num_hidden_neurons_per_slice + \
                sublocation_idx * num_slices_per_class + \
                slice_group_idx[i]
            hidden_connectivity[idx]["fan_out_synapse_indices"].append(fan_out_synapse_idx)
##########################################################################
...
###################### Establish Input Layer Connectivity ##################
    ...
    for idx in range(num_input_neurons):
        ...
        for i in range(len(sublocation_idx_list)):
            ...
            for fan_out_neuron_idx in fan_out_neuron_list:
                j = fan_out_neuron_idx - num_input_neurons
                slice_idx = hidden_connectivity[j]["slice_idx"]
                fan_out_synapse_idx = \
                    num_input_neurons + (
                            (((sublocation_idx * num_hidden_slices) +
                              slice_idx
                              ) * num_edge_maps) + edge_map_idx
                    ) * F_hidden ** 2 + kernel_element_idx
                input_connectivity[idx]["fan_out_synapse_indices"].append(fan_out_synapse_idx)
#############################################################################
...
#############################################################################################
        ...
        pd.DataFrame(
            {
                ...
                'Neuron Index': input_neuron_list,
                'Edge Map Index': [input_connectivity[i]["edge_map_idx"] for i in range(num_input_neurons)],
                'Sublocation Index': [input_connectivity[i]["receptive_field_info"]["sublocation_idx"] for i in
                                      range(num_input_neurons)],
                'Kernel Element Index': [input_connectivity[i]["receptive_field_info"]["kernel_element_idx"] for i in
                                         range(num_input_neurons)],
                'Fan-in Neuron Indices': [None for i in range(num_input_neurons)],
                'Fan-in Synapse Indices': [input_connectivity[i]["fan_in_synapse_addrs"] for i in
                                           range(num_input_neurons)],
                'Fan-out Neuron Indices': [input_connectivity[i]["fan_out_neuron_indices"] for i in
                                           range(num_input_neurons)],
                'Fan-out Synapse Indices': [input_connectivity[i]["fan_out_synapse_indices"] for i in
                                            range(num_input_neurons)]
        ...
        pd.DataFrame(
            {
                'Layer Index': [1] * num_hidden_neurons,
                'Neuron Index': hidden_neuron_list_flattened,
                'Depth Index': [hidden_connectivity[i]["slice_idx"] for i in range(num_hidden_neurons)],
                'Sublocation Index': [hidden_connectivity[i]["sublocation_idx"] for i in range(num_hidden_neurons)],
                'Null Field0': [None] * num_hidden_neurons,
                'Fan-in Neuron Indices': [hidden_connectivity[i]["fan_in_neuron_indices"] for i in
                                          range(num_hidden_neurons)],
                'Fan-in Synapse Indices': [hidden_connectivity[i]["fan_in_synapse_addrs"] for i in
                                           range(num_hidden_neurons)],
                'Fan-out Neuron Indices': [hidden_connectivity[i]["fan_out_neuron_indices"] for i in
                                           range(num_hidden_neurons)],
                'Fan-out Synapse Indices': [hidden_connectivity[i]["fan_out_synapse_indices"] for i in
                                            range(num_hidden_neurons)]
            }
        )
    hidden_layer_df.name = "Hidden Layer"
        ...
        pd.DataFrame(
            {
                ...
                'Neuron Index': output_neuron_list,
                'Category Index': [output_connectivity[i]["category_idx"] for i in range(num_output_neurons)],
                'Fan-in Slice Indices': [output_fan_in_slice_idx[i] for i in range(num_output_neurons)],
                'Null Field2': [None] * num_output_neurons,
                'Fan-in Neuron Indices': [output_connectivity[i]["fan_in_neuron_indices"] for i in
                                          range(num_output_neurons)],
                'Fan-in Synapse Indices': [output_connectivity[i]["fan_in_synapse_addrs"] for i in
                                           range(num_output_neurons)],
                'Fan-out Neuron Indices': [None] * num_output_neurons,
                'Fan-out Synapse Indices': [None] * num_output_neurons
            }
    ...
    input_layer_df.to_excel(writer, sheet_name='Sheet1', index=False, startrow=0, startcol=0)
    hidden_layer_df.to_excel(writer, sheet_name='Sheet1', index=False, startrow=input_layer_df.shape[0] + 5, startcol=0)
    output_layer_df.to_excel(writer, sheet_name='Sheet1', index=False,
                             startrow=input_layer_df.shape[0] + 5 + hidden_layer_df.shape[0] + 4, startcol=0)
    worksheet1 = writer.sheets['Sheet1']
    # worksheet.merge_range('A1:I1', input_layer_df.name, merge_format)
    # worksheet.merge_range('A69:I69', hidden_layer_df.name, merge_format)
    # worksheet.merge_range('A119:I119', hidden_layer_df.name, merge_format)
    ...
    return (input_connectivity, hidden_connectivity, output_connectivity, ConnectivityTable,
            WeightRAM, PotentialRAM, writer)
